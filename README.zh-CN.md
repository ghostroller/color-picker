...
冻结模式使用局部快照，便于选中细小目标，不受后续画面变化影响。鼠标在像素格内移动时，信息栏实时显示当前像素的 HEX、原始屏幕 X/Y 坐标和放大倍率。点击浮窗边框或信息栏不会取色。取色期间鼠标点击和滚轮由取色工具处理，结束后恢复正常输入。

![冻结像素格、色值、原始坐标与放大倍率](docs/images/ui-frozen.png)
