...
3. 首次手动启动会显示简短指引，之后可在系统托盘（包括隐藏图标区域）找到 Color Picker。登录 Windows 时启动保持安静，不弹出指引。
4. 按 **Ctrl + Alt + C**，移动到目标颜色，**左键单击**确认。
5. 在结果窗口复制 HEX、RGB、CSS RGB 或 HSL；点击**重新取色**可继续选取。

应用与安装器支持**简体中文和英文**。应用初始跟随 Windows 显示语言：中文系统使用简体中文，其他语言使用英文。在设置中选择**语言**并点击**应用**即可立即切换，无需重启。

目标平台为 **Windows 11 x64**，Windows 10 22H2 x64 尚未完成完整兼容性验证。当前程序和安装包尚未签名，验证范围见[已知限制](docs/known-limitations.md)。
...
冻结模式使用局部快照，便于选中细小目标，不受后续画面变化影响。鼠标在像素格内移动时，信息栏实时显示当前像素的 HEX、原始屏幕 X/Y 坐标和放大倍率。色值左侧的色块贴齐信息栏左边缘，随鼠标悬停的像素同步更新，不增加窗口宽度。点击浮窗边框或信息栏不会取色。取色期间鼠标点击和滚轮由取色工具处理，结束后恢复正常输入。

屏幕边缘与中央使用相同尺寸的冻结浮窗，仅在工作区空间不足时缩小。初次冻结会按浮窗位置调整局部快照，让鼠标仍指向冻结前的原始像素；这适用于鼠标落在有效像素格内的情况。右边框、底部信息栏和任务栏区域仍不取色。后续缩放受到快照边界限制，不能保证所有位置的像素始终留在鼠标下。

![中文冻结预览：悬停色块、HEX 色值、原始坐标与放大倍率](docs/images/ui-frozen.png)

取色期间重复激活不会叠加会话。托盘菜单显示当前快捷键及其在 Windows 中的注册状态。关闭结果窗口后，程序仍驻留托盘；如需完全退出，请右键托盘图标并选择**退出**。
...
主复制按钮使用设置中的默认格式，初始为 HEX。复制成功时，对应按钮短暂显示**已复制**，正常窗口布局保持稳定；只有失败时才展开错误信息。结果窗口支持 **Tab**、**Enter** 和 **Esc** 进行键盘导航和关闭。

**自动复制和快速取色均默认关闭。**普通自动复制仍会打开结果窗口。开启**快速取色**后，每次确认颜色都会复制默认格式，成功时不弹出结果窗口、不改变当前应用的焦点，方便直接粘贴；复制失败才显示结果供重试。开启快速取色时会保留普通自动复制的偏好，关闭快速取色后恢复使用该偏好。
...
| 设置 | 选项 | 默认值 |
| --- | --- | --- |
| 语言 | 简体中文 / English | 跟随 Windows，非中文使用英文 |
| 全局快捷键 | Ctrl 和 / 或 Alt，可选 Shift，加 A–Z、0–9 或 F1–F11 | Ctrl + Alt + C |
| 默认复制格式 | HEX、RGB、CSS RGB、HSL | HEX |
| 取色后自动复制 | 开 / 关 | 关 |
| 快速取色：复制后不打开结果窗口 | 开 / 关；始终复制默认格式 | 关 |
| 边框粗细 | 0–6 DIP；0 隐藏边框 | 2 DIP |
| 取色背景透明度 | 0–80%；0 为不透明 | 35% |

修改快捷键时，先勾选修饰键，再点击主键按钮并按下新字母、数字或功能键。按 **Esc** 或将焦点移出控件会取消监听。F12 为保留键，不可使用。

设置窗口按显示器可用高度调整大小。屏幕较小或缩放较高时，内容可滚动，底部的**应用**按钮保持可见。拖动边框与透明度滑块时，示例颜色预览立即更新；预览使用合成颜色，点击应用才会保存修改。

点击**应用**保存，再关闭设置窗口即可继续取色。语言保存成功后，设置、托盘菜单及后续取色窗口立即使用新语言，设置页保留当前滚动位置。设置打开期间触发取色，会恢复已有设置窗口并说明暂停取色的原因，保留当前编辑内容。未应用就关闭会放弃修改。新快捷键冲突或保存失败时，旧设置和原语言继续有效。取色浮窗与结果窗口使用一致的右侧和底部边框；透明度仅影响取色浮窗的信息区背景，色块和放大像素始终保持不透明。
...
首次指引使用独立的 `%LOCALAPPDATA%\color-picker\welcome-v1.seen` 文件记录是否已显示，不会为此改写配置。
...
快捷键无响应时，可先查看托盘菜单中的注册状态，再从托盘取色，并关闭打开的设置窗口。快捷键冲突、配置恢复、剪贴板失败和可选文件日志见[故障排查](docs/troubleshooting.md)（英文）；完整测试状态见[已知限制](docs/known-limitations.md)和[验证记录](docs/validation.md)。
...
自动化启动时可传入 `--no-onboarding`，跳过首次指引且不将其标记为已显示，下次普通手动启动仍可显示。`--startup` 同样跳过指引，已有实例时安静退出。可选配置字段 `quick_pick` 默认为 `false`，旧配置缺少此字段时也保持关闭。

可选配置字段 `language` 仅接受 `"zh-CN"` 和 `"en"`。旧配置缺少该字段时使用 Windows 语言默认值，应用保存后记住所选语言。翻译使用 `src/app/i18n.rs` 的 `tr` / `tr_format!` 成对维护，中英格式字符串均由编译器检查，活动语言保存在 UI 线程中。Windows 原生错误的详细信息可能使用操作系统语言。
