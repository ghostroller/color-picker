...
| 放大镜视口 | 首次放大时按 `240 DIP` 方形布局并限制在工作区内 | 本次冻结期间窗口位置和视口不随鼠标移动；像素格仍按整数物理像素绘制 |
...
本次会话建立固定窗口和视口位置，尽量以首次滚轮位置为图像中心，整体限制在当前显示器工作区内。视口约 240 DIP，实际像素尺寸由该窗口 DPI 与可用工作区确定；底部仅保留 36 DIP 的颜色与倍率信息栏。实时预览为 208×58 DIP，操作说明统一放在设置页。
