...
| 冻结范围 | 最多 `65 × 65` 个源物理像素 | 靠近屏幕边缘时裁剪，不跨显示器拼接 |
| 放大档位 | `4× → 8× → 16× → 32×` | 每个源像素对应 4/8/16/32 个显示物理像素 |
| 缩小到最低档以下 | 返回实时预览 | 丢弃旧冻结图像；下次放大重新截取 |
| 放大镜视口 | 首次放大时按 `320 DIP` 方形布局并限制在工作区内 | 本次冻结期间窗口位置和视口不随鼠标移动；像素格仍按整数物理像素绘制 |
| 默认格式 | 大写 HEX，例如 `#409EFF` | 结果窗口同时显示其他格式 |
| 自动复制 | 默认关闭 | 默认只弹窗；用户可在设置中开启 |
| 历史/开机启动 | 第一版不做 | 先完成低开销取色闭环 |

第一版快捷键设置支持 Ctrl/Alt/Shift 加字母、数字或 F1–F11，至少包含 Ctrl 或 Alt。暂不提供 Win 组合键和任意复杂按键录制，避免系统保留键与设置实现扩张。`RegisterHotKey` 有冲突和保留键限制，F12 不应注册；使用 `MOD_NOREPEAT` 抑制按键自动重复。[S05]
...
候选发出后不能继续产生第二次选择。等待主线程决定期间仍处理必要的释放事件，主线程不通过同步调用卡住回调。冻结模式点击视口外、边框或无效留白时，候选不成立：继续取色，不“夹到最近像素”并悄悄选择。
...
取事件位置 P 与所在显示器 M
    ↓
构造 [Px-32, Py-32, Px+33, Py+33) 并与 M.rcMonitor 求交
    ↓
...
释放 Live 的采样 DC/位图，不再保留持续采样器
    ↓
进入 Frozen，初始化视口与倍率，显示放大镜
```

快照矩形只在当前显示器内裁剪，不把虚拟桌面的外接矩形当成每个像素都有效的图像。这样显示器间空隙、混合 DPI 边界和不同显示设备不会悄悄产生“补黑可选像素”。

显示边缘的快照可以小于 65×65。保存真实 `origin/width/height`，原始鼠标点在缓存中的位置由 `P - origin` 得出，不假定永远在 `(32, 32)`。

### 10.2 放大镜视口规则

本次会话建立固定窗口和视口位置，尽量以首次滚轮位置为图像中心，整体限制在当前显示器工作区内。视口约 320 DIP，实际像素尺寸由该窗口 DPI 与可用工作区确定；文字栏额外保留高度。
...
实时采样间隔、快照尺寸和倍率在第一版保留为代码常量，不暴露大量会破坏体验/性能的设置。配置 UI 仅提供三个修饰键复选框、主键下拉框、默认格式下拉框和自动复制复选框。
...
| W11 | 鼠标在放大镜外/文字栏/留白处点击 | 不误选；继续等待有效选择 |
