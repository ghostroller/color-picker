...
| 放大镜视口 | 首次按快照 `4×` 的实际物理范围收紧，每轴最多 `240 DIP` 并限制在工作区内 | 无外围边距，裁剪快照可为矩形；本次冻结期间窗口与视口固定，像素格按整数物理像素绘制 |
...
信息背景采用局部软件磨砂：在捕获排除成功时，仅截取信息条背后的区域，模糊后混入约 84% 深色。
缓存按屏幕位置复用，移动 / DPI 或尺寸改变才更新；静止和冻结时不跟随后方动画。
效果失败回退纯色，不改变取色数据。下、右各有 1 物理像素边线，不扩充边距。
...
本次会话建立固定窗口和视口位置，尽量以首次滚轮位置为图像中心，整体限制在当前显示器工作区内。初始视口每轴按快照像素数 ×4 收紧，最多 240 DIP；极窄缓存至少保留 32 个物理像素以容纳最高倍率。视口无外围边距，底部仅保留 24 DIP 的颜色与倍率信息栏。实时预览为 168×38 DIP，色块贴边，操作说明统一放在设置页。
...
实时采样间隔、快照尺寸和倍率在第一版保留为代码常量，不暴露大量会破坏体验/性能的设置。配置 UI 提供三个修饰键复选框、点击后监听的主键按钮、默认格式下拉框和自动复制复选框，以及简短取色操作说明。
