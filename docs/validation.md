...
## unsafe 边界重构 · 2026-09-27

用户后续将本次发版验收范围调整为基础测试，并授权 GitHub CI 构建及发布；以下专项缺口保留记录，不阻塞本次发布。

R01—R07 的实现、安全契约、命令与退出码、原生资源回归和未测范围见
[实施报告](unsafe-refactoring-report-2026-09-27.zh-CN.md)及
[安全边界说明](unsafe-boundaries.md)。最终完整标准验证通过：168 项默认测试、严格全目标 Clippy、
MSVC Release、实际 manifest 与 PMv2；另有 19 项单独筛选的原生桌面/资源检查通过。
首次英文标题回归已修复并复测，历史失败日志保留；机器可读[证据摘要](measurements/unsafe-refactoring-20260927-summary.json)包含命令、退出码和日志指纹。
独立进程四窗口/表面重复创建销毁的资源检查通过；baseline/final 性能比较因已有用户实例及
诊断未启用而 BLOCKED，不能以历史测量或局部资源稳定代替本次完整性能验收。
二次逐项核对还确认资源构造失败矩阵、DIB NullStorage 释放次数、SaveDC 调用方回退/提前返回、未知通知实际默认分发链的测试覆盖未补齐；
这些是本机可补的测试工作，已列入报告第 7 节，不归为环境阻塞。

## 89021c1 审查复核 · 2026-09-22

逐项结论、修复范围、本轮检查结果及尚未执行的独立桌面验收见
[审查复核记录](review-89021c1-follow-up.md)。历史性能与交互记录只代表各自记录的构建，
不作为本次修改的动态性能或真实复制验收结果。
...
## 2026-09-22：边缘冻结与中央布局对齐试用版

本轮先创建隐藏窗口取得目标 DPI、确定标准窗口位置，再反推初始 4× 所需的源视图，
最后截取一次最多 65×65 的快照。边缘通过移动捕获区域保留完整尺寸，窗口不再随
边缘裁剪缩窄。初次冻结时，鼠标在有效像素格内会继续命中冻结前的原始屏幕像素。
信息栏、右边框、工作区外的鼠标不纳入原地命中保证；后续倍率切换仍按缓存边界约束。

- `scripts/verify-windows.ps1` 全部通过：格式、Clippy、133 项默认测试、x64 Release、
  嵌入 manifest、主程序与像素夹具 PerMonitorV2 检查。
- 新增 11 项几何和映射回归，覆盖原 x=20 漂移场景、四边四角、负坐标、不同 DPI
  尺寸、非整倍视口、小显示器、整数极值及缓存不足回退。
- GDI 离屏绘制回归覆盖 96 / 120 / 144 / 168 / 192 DPI，验证边缘初次高亮、
  信息栏与点击映射均指向原始焦点；后续缩放、无效区与重入仍保持一致。
- `cargo test --locked --test windows_selection_ui -- --ignored --test-threads=1 --nocapture`
  在具备桌面访问的执行环境中 3 项通过：既有倍率与控件测试、中心/左/上/左上窗口
  尺寸与初始颜色一致、截图失败时隐藏 1×1 窗口释放。新增用例使用合成快照，
  不发送系统输入、不修改剪贴板。普通沙箱在读取鼠标坐标时返回 `0x80070005`。
- `ui-preview` 的 `frozen-edge` 已改为工作区左上角的真实布局位置。本轮独立导出
  在当前环境返回 `0x80070005`，未完成目视截图检查，文档旧图标记为历史预览。

0.2.3 版本更新后重新运行完整标准检查通过，EXE FileVersion / ProductVersion 均为 `0.2.3`。
Release：`target/x86_64-pc-windows-msvc/release/color-picker.exe`，
SHA256：`2BAD6ABB476D63B26CB4537FDAC845AFAAC479FE83E1AF741A91DB5170ABC39A`。
尚未执行真实鼠标从 Live 经滚轮进入 Frozen 的端到端验收；上述局部验证不将
W01–W11 等完整发布矩阵改为 PASS。
