...
## 0.2.1 精简发布包与许可 · 2026-09-21

完整 `verify-windows.ps1` 通过：98 项默认测试、Clippy、x64 Release、内嵌清单与 DPI 检查。
官方 Inno Setup 6.7.3 编译及独立安装生命周期测试通过；新安装为 5 个 payload 文件和 2 个卸载文件。
两个 README 的正文完整保留，仅包内文档链接改为固定源码网址；ZIP 文件清单、许可及 SHA256 核对通过。
102 份第三方源材料逐份核对原文及 SHA256，完全相同的文本去重，保留 Rust 原始版权 HTML。
旧文件清单的 140 个指纹与 0.1.0/0.2.0 发布包核对一致；升级测试确认匹配文件及空目录删除，
修改/未知文件、越界路径、当前 README，以及文件父目录、安装根目录、上级目录的 junction 均受保护。
隔离测试不启动常驻应用，用户配置指纹前后相同；日志仅保存在 `target/installer-smoke/`。

## 安装与升级 · 2026-09-21

完整 `verify-windows.ps1` 通过：87 项默认测试、Clippy、x64 Release、
内嵌 manifest 和应用/像素夹具 DPI 检查。新增覆盖自动启动参数互斥、
退出命令安装路径隔离、实例已持有 mutex 而窗口未就绪的有界等待。

实际 Inno Setup 6.7.3 编译通过（官方简中翻译固定在仓库中）。
`scripts/test-installer.ps1` 使用独立 AppId、带中文/空格的测试目录和真实 payload，
验证默认安装、启动/桌面选项启用、合成 0.1.0 → 0.1.1 升级保留选项、
...
纵向缩放跟随修复：真实 GDI 离屏绘图复现 8× 时应显示缓存行 7 却画出行 26，
导致鼠标偏上 / 偏下放大时画面反向偏移。锚点映射本身正确，修复仅调整传给 GDI 的
纵向行切片和 DIB 高度，源 y 固定为 0，保持行序、行宽和横向裁剪。
回归在修复前失败、修复后通过：上下两个鼠标锚点，各经历 4×→8×→16×→32×→16×→8×→4×，
逐档比较上 / 中 / 下、左 / 中 / 右的真实绘制像素与缓存命中结果。
完整标准检查共 84 项默认测试通过；此回归不显示窗口、不采集桌面、不合成系统输入。
