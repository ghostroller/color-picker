...
结果窗口在 Windows 10 / 11 上统一采用直角、无系统阴影的扁平样式，只保留上述自定义边框。
通过关闭该窗口的 DWM 非客户区渲染，并在 Windows 11 上显式禁止圆角实现；
Windows 10 不支持的圆角 / 边框颜色属性按可选项处理。
...
设置页按内容区实际宽度保持左右等边距，仅需要滚动时占用原生滚动条空间；
语言标签与下拉框按实际控件高度居中对齐。底部简短提示使用不可选中的静态文字，
长诊断信息仍可通过滚动详情完整阅读。
...
语言不变时，应用后仅更新保存提示，复用现有文案、字体和预览，避免整页重排重绘造成闪动。
...
冻结像素区取消外围边距，每轴按 240 DIP 随显示器缩放，再向下取完整 4× 像素格；
125% 为 300×300、175% 为 420×420 物理像素，屏幕中央与边缘尺寸一致。
先确定窗口位置，再按初始 4× 倍率规划对应的局部快照（175% 为 105×105），
使有效像素格内的鼠标在初次冻结时仍命中原像素。工作区不足时缩小；捕获每轴最多 512 个源像素。
信息栏保持 28 DIP 单行，HEX / 倍率与坐标使用随 DPI 缩放的 13 / 10 DIP 字号；
色值左侧的色块扣除底部边框后保持正方形，贴齐信息栏左侧，随当前 hover 像素同步更新；
左列显示该像素的 HEX 与倍率，右列显示原始物理屏幕 X/Y；
移到信息栏、边框或无效像素时显示占位符。窗口宽度跟随像素区，不为文字扩宽，
不增加两侧留白。完整窗口优先使用正常字号；工作区异常狭窄时才使用紧凑坐标和较小字体，
始终保持单行，不主动省略倍率。极窄时色块等比例缩小并垂直居中，文字按可用范围裁剪。
可用视口每轴至少需要 32 个物理像素，以支持最高倍率；缩放时保留完整整数像素格所需的
...
输入钩子、动画或额外运行时依赖。最新 Release 的分阶段进程占用与历史环境差异
见[2026-09-21 性能复测](performance-running-app.md)；短期测量不替代长时间工作流验收。

## 本机检查

- 2026-09-26 结果窗口样式统一：Windows 11 build 26200 已检查真实预览的直角、自定义
  右 / 下边框及无旧式标题栏；禁用 DWM 后同时处理非客户区绘制和激活消息，防止经典边框覆盖内容。
  完整 `scripts/verify-windows.ps1` 与既有结果控件冒烟通过，包含标题拖动命中、最小化、
  恢复、关闭和只读色值；Windows 10 尚未进行本轮实机复测。
- 2026-09-22 高 DPI 回归：中英文覆盖 100% / 125% / 150% / 175% / 200% / 250% / 300%，
  240 DIP 窗口、0 / 2 / 6 DIP 边框及 4× / 8× / 16× / 32×；
  检查 HEX、倍率、坐标完整单行显示且不裁剪或重叠；离屏 GDI 确认色块宽高相等。
  字号回归检查真实字体高度，防止仅通过缩小字体满足完整性检查。
  175% 主屏原生预览为 420×469 物理像素，色块 45×45，HEX / 倍率 / X/Y 使用正常字号单行显示。
  完整 `scripts/verify-windows.ps1` 和 3 项 `windows_selection_ui` 桌面冒烟通过；
  真实 GDI 捕获验证 75×75 / 105×105 / 120×120 快照与超限拒绝。
  实际鼠标跨屏操作仍待体验验收。
...
- 语言选择框使用独立的中英字体，仅在 DPI 改变时重建，避免切换到英文后首次展开时
  把“简体中文”滚出可见区域。新增下拉框回归在旧实现上复现，在修复后通过；
  覆盖中英往返切换后首次展开的两项完整可见、选择值保留和中文字形支持。
...
以下是旧版边缘窄窗口的历史预览。2026-09-22 的试用版本已改为与中央相同尺寸，
可用 `frozen-edge` 预览工作区左上角的当前布局：
