...
冻结像素区取消外围边距，每轴统一取 260 物理像素、240 DIP 和可用工作区的最小值，
再向下取完整 4× 像素格；屏幕中央与边缘尺寸一致，仅工作区不足时缩小或变为矩形。
先确定窗口位置，再规划最多 65×65 的局部快照，使有效像素格内的鼠标在初次冻结时仍命中原像素。
信息栏固定为 28 DIP 单行，
色值左侧的色块贴齐信息栏左边缘，随当前 hover 像素同步更新；
左列显示该像素的 HEX 与倍率，右列显示原始物理屏幕 X/Y；
移到信息栏、边框或无效像素时显示占位符。窗口宽度跟随像素区，不为文字扩宽，
不增加两侧留白。信息栏按实际文字宽度选择字号和间距；小工作区使用紧凑坐标，
